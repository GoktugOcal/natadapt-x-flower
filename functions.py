            ...
            if kept_filter_idx is None:
                ...
                if layer_param_name == WEIGHTSTRING: #WEIGHTSTRING == layer_param_name:                    
                    ...
                    print('    simplify_model> simplify Conv layer {}: output channel biases {}'.format(layer_name_str, 
                          len(kept_filter_idx)))
                    print(kept_filter_idx)
                    skd = torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx])
                    setattr(layer, layer_param_name, torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx]))
                else:
